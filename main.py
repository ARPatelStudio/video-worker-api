...
import os
import shutilfrom fastapi import FastAPI, Form, File, UploadFile, BackgroundTasks
from fastapi.responses import FileResponse
from pydantic import BaseModel
import os
import shutil
...
# ====================================================
# AUDIO AUTO FIX USING FFMPEG (Brilliant Addition! 🚀)
# ====================================================
def ensure_valid_audio(input_path):
    fixed_path = input_path + "_fixed.wav"
    cmd = [
...
def create_merged_video_task(job_id, images_paths, audio_path, subtitles):
    try:
        ...
        # Force fix audio before giving it to MoviePy
        audio_path = ensure_valid_audio(audio_path)
        audio_clip = AudioFileClip(audio_path)
        total_duration = audio_clip.duration
        num_images = len(images_paths)
